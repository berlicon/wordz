...
namespace Wordz.UT {
    ...
    public class BCCommon_en_ru_Test : DbTransactionalFixture {
        ...
        public void RemoveWrongSymbols() {
            string text =
                BCCommon.WrongSymbolsExample + "помощник" +
                BCCommon.WrongSymbolsExample + "оружие" +
                BCCommon.WrongSymbolsExample;
            string processedText = BCCommon.RemoveWrongSymbols(text, (int) Language.Russian);

            int indexhelper = processedText.IndexOf("помощник");
            int indexweapon = processedText.IndexOf("оружие");
        ...
        public void NormalizeText() {
            string text = "   помощник   оружие   ";
            string processedText = BCCommon.NormalizeText(text);
            Assert.AreEqual("помощник оружие", processedText);
        }

        [Test]
        public void CreateWordsCollection() {
            string text = "помощник оружие помощник оружие помощник назад";
            Hashtable words = BCCommon.CreateWordsCollection(text, false, (int) Language.Russian);
            Assert.AreEqual(3, words.Count);

            WordElement helperElement = (WordElement) words["помощник"];
            WordElement weaponElement = (WordElement) words["оружие"];
            WordElement abackElement = (WordElement) words["назад"];

            Assert.AreEqual("помощник", helperElement.Original);
            ...
            Assert.AreEqual("оружие", weaponElement.Original);
            ...
            Assert.AreEqual("назад", abackElement.Original);
        ...
        public void ShowProcessTextLearnFirstWord() {
            ...
            string expectedText =
                ...
                "оружие arms" + Environment.NewLine +
                "помощник assistant" + Environment.NewLine;
        ...
        public void ShowProcessTextNativeFirstWord() {
            ...
            string expectedText =
                ...
                "arms оружие" + Environment.NewLine +
                "assistant помощник" + Environment.NewLine;
            Assert.AreEqual(expectedText, processedText);
        }
        ...
        public void GetWordsInfoCheckMinFrequency() {
            ...
            Assert.IsNotNull(info.ProcessedWords["помощник"]);
            Assert.IsNotNull(info.ProcessedWords["оружие"]);
            Assert.IsNotNull(info.ProcessedWords["назад"]);
        ...
        public void GetWordsInfoCheckNotUseWellKnownWords() {
            ...
            WordElement element1 = new WordElement("помощник", "", 1, 0, 0);
            element1.WellKnown = true;
            words["помощник"] = element1;

            WordElement element2 = new WordElement("оружие", "", 1, 0, 0);
            element2.WellKnown = false;
            words["оружие"] = element2;
        ...
        public void GetWordsInfoCheckNotUseNotKnownWords() {
            ...
            WordElement element2 = new WordElement("оружие", "оружие", 1, 0, 0);
            words["оружие"] = element2;
            ...
            Assert.AreEqual(1, info.WordsCount);
            Assert.IsNull(info.ProcessedWords["помощник"]);
            Assert.IsNotNull(info.ProcessedWords["оружие"]);
        }
